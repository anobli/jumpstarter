...
async def PtyAdapter(
    *,
    ...
    consumers — picocom, pyserial, or any tool expecting a real serial
    device — can attach to. The bridge survives transient disconnects of the
    ...
    # The slave must be raw: left in cooked mode with ECHO, every byte
    # written by the bridge would be echoed back through the master and
    # re-sent upstream, forming a feedback loop. The master setraw is
    # belt-and-suspenders (termios on the master side has no real effect on
    # the data path).
