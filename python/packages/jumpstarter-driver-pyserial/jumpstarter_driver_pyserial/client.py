...
class PySerialClient(DriverClient):
    ...
    def pty(self, symlink_path: Optional[str] = None):
        ...
        for tools that expect a real serial device — picocom, pyserial,
        etc. The slave path stays stable across release()/acquire() cycles
        on the server side, so a brief flasher take-over is seen as a pause
        rather than EOF.
    ...
    def cli(self):  # noqa: C901
        ...
        def pty_cmd(symlink, quiet):
            ...
            blocks until Ctrl+C. Point any serial tool at the path:
